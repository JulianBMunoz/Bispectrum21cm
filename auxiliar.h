...
double ****create_4D_array(unsigned n1, unsigned n2, unsigned n3, unsigned n4){
   unsigned i;
   double ****matrix = (double ****) calloc(n1, sizeof(double ***));
   if (matrix == NULL){
      fprintf(stderr, "Error in create_3D_array: unable to allocate memory\n");
   ...
   for (i = 0; i < n1; i++)  matrix[i] = create_3D_array(n2,n3,n4);
...
void free_4D_array(double ****matrix, unsigned n1,unsigned n2,unsigned n3){
    unsigned i;

    for (i = 0; i < n1; i++){
    	free_3D_array(matrix[i],n2,n3);
    }
